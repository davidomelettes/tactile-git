...
(SELECT created FROM ticket_responses WHERE ticket_id = t.id ORDER BY created DESC LIMIT 1) AS last_response_time,
(SELECT owner FROM ticket_responses WHERE ticket_id = t.id ORDER BY created DESC LIMIT 1) AS last_response_by
  FROM tickets t
