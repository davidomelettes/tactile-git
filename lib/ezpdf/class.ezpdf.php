...
//  $y-=$gap-$decender;
  ...
  while ($ok==0){
    foreach($cols as $colName=>$colHeading){
      ...
      //$this->filledRectangle($pos[$colName],$this->y-$this->getFontHeight($size)+$this->getFontDecender($size),$maxWidth[$colName],$this->getFontHeight($size));      
      //$this->restoreState();
      if(isset($optionsAll['headCol'])) {
	$old_col = $this->currentColour;
	$this->setColor($optionsAll['headCol'][0],$optionsAll['headCol'][1],$optionsAll['headCol'][2]);
      }
      $this->ezText($colHeading,$size,array('aleft'=> $pos[$colName],'aright'=>($maxWidth[$colName]+$pos[$colName]),'justification'=>$justification));
      if(isset($optionsAll['headCol'])) {
	$this->setColor($old_col['r'],$old_col['g'],$old_col['b']);
      }
      $dy = $y-$this->y;
      if ($dy>$mx){
        $mx=$dy;
...
//        $x1=$x1+$dm;
        ...
        $y = $this->ez['pageHeight']-$this->ez['topMargin']-20;
        $y0=$y+$decender;
        $mx=0;
        if ($options['showHeadings']){
  ...
  //            $this->ezNewPage();
		...
		if (isset($options['cols'][$colName]) && isset($options['cols'][$colName]['textCol'])) {
			$this->setColor($options['cols'][$colName]['textCol'][0],$options['cols'][$colName]['textCol'][1],$options['cols'][$colName]['textCol'][2]);
		}
		else {
			$this->setColor($options['textCol'][0],$options['textCol'][1],$options['textCol'][2]);
		}
	...
	if (isset($options['minHeight'])) {
              if ($dy < $options['minHeight']) {
                      $dy = $options['minHeight'];
		}
	}
