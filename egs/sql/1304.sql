...
CREATE VIEW tickets_overview AS SELECT t.id, t.summary, (tpc."index"::text || '-'::text) || tpc.name::text AS client_ticket_priority_id, (tpi."index"::text || '-'::text) || tpi.name::text AS internal_ticket_priority_id, (tsc."index"::text || '-'::text) || tsc.name::text AS client_ticket_severity_id, (tsi."index"::text || '-'::text) || tsi.name::text AS internal_ticket_severity_id, tstc.name AS client_ticket_status_id, tsti.name AS internal_ticket_status_id, tc.name AS ticket_category_id, tq.name AS ticket_queue_id, (((((((COALESCE(p.title, ''::character varying)::text || ' '::text) || p.firstname::text) || ' '::text) || COALESCE(p.middlename, ''::character varying)::text) || ' '::text) || p.surname::text) || ' '::text) || COALESCE(p.suffix, ''::character varying)::text AS originator_person_id, c.name AS originator_company_id, to_char(t.created, 'IYYY-MM-DD HH24:MI:SS'::text) AS created, to_char(t.lastupdated, 'IYYY-MM-DD HH24:MI:SS'::text) AS lastupdated, tq.usercompanyid
   FROM tickets t
