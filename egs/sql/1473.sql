...
t.internal_ticket_severity_id, 
tstc.name AS client_ticket_status,
tstc.status_code AS client_status_code,
t.client_ticket_status_id,
tsti.name AS internal_ticket_status, 
tsti.status_code AS internal_status_code,
t.internal_ticket_status_id, tc.name AS ticket_category,
t.ticket_category_id, 
tq.name AS ticket_queue, t.ticket_queue_id, 
p.firstname || ' ' || p.surname AS originator_person, 
t.originator_person_id, 
c.name AS originator_company,
t.originator_company_id,
to_char(t.created, 'IYYY-MM-DD HH24:MI:SS'::text) AS created, 
to_char(t.lastupdated, 'IYYY-MM-DD HH24:MI:SS'::text) AS lastupdated, 
tq.usercompanyid
   FROM tickets t
